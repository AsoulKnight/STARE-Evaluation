...
from models.unetr_2d import UNETR2D
from utils.metrics import dice_score, dice_loss, iou_score, format_time, soft_dice_score, FocalLoss
...
EXPERIMENT_NAME = "unetr_train_pos10_lr5e-5_epoch20_wd1e-5_bs2_dicebcefocal"
RESUME = False

BASE_SAVE_DIR = "C:/1.lwBrown/Lee Lab/Evaluation_STARE/experiments_unetr"
BASE_MODEL_DIR = os.path.join(BASE_SAVE_DIR, "unetr_base_train")
...
def train_one_epoch(model, loader, optimizer):
    ...
    for i, (images, masks) in enumerate(loader):
        ...
        assert masks.max() <= 1, "Mask values are not normalized!"
        ...
        loss = loss = bce_loss(outputs, masks) + dice_loss(outputs, masks) + focal_loss(outputs, masks)
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for images, masks in loader:
            ...
            dice = soft_dice_score(probs, masks)
...
def train_single_split():
    ...
    # Model
    model = UNETR2D(img_size=512).to(DEVICE)
    ##### add bias initialization ####
    model.final.bias.data.fill_(-2.0)
    ################################
